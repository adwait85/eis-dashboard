...
import { getFirestore, doc, setDoc, onSnapshot, addDoc, collection, query, where, orderBy, limit, serverTimestamp, getDocs } from 'firebase/firestore';
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, ScatterChart, Scatter } from 'recharts';
import { Home, Beaker, History, Wifi, Play, Save, Loader2, AlertCircle, CheckCircle, Sparkles, Upload, XCircle, Droplets, Leaf, FlaskConical, Edit3, Map, Send, User, Bot } from 'lucide-react';
...
/* ⚠️ Replace with your own Firebase config in production */
const firebaseConfig = {
  apiKey: "AIzaSyAKsju4i7rTicmHE4PXGmCr0wKPqzaCH4A",
...
function Button({ text, icon, onClick, disabled, className = '', type = 'button' }) {
  return (
    <button
      ...
      `}
    >
      {disabled ? <Loader2 className="animate-spin h-5 w-5" /> : icon}
      <span>{text}</span>
    </button>
  );
}
...
function ChatBubble({ role, content }) {
  if (role === 'user') {
    if (typeof content === 'string' && content.includes("Here is the NEW sweep data to analyze:")) return null;
    return (
  ...
  return (
    <div className="flex justify-start">
      <div className="bg-gray-900 p-3 rounded-lg max-w-lg border border-gray-700">
        ...
        {isStructuredReport ? <AnalysisReport report={content} /> : <p className="text-white whitespace-pre-wrap">{content}</p>}
      </div>
    </div>
  );
}
...
function TomographyHeatmap({ data, metric }) {
  ...
  const getColor = (val) => {
    ...
    const h = (1.0 - norm) * 240; // 0 (red) -> 240 (blue)
    return `hsl(${h}, 100%, 50%)`;
  };
      ...
      {data.map((d, i) => {
        ...
        return (
          <div
            ...
            style={{ left: `${left}%`, top: `${top}%`, backgroundColor: color, boxShadow: `0 0 8px ${color}` }}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    script.onerror = () => console.error("Failed to load PapaParse script.");
  ...
  useEffect(() => {
    ...
    try {
      const stateUnsub = onSnapshot(doc(db, stateDocPath), (docSnap) => {
        if (docSnap.exists()) {
  ...
  const sendDeviceCommand = async (commandName, payload = {}) => {
    ...
    try {
      await setDoc(doc(db, stateDocPath), {
        command: { name: commandName, ...payload, timestamp: serverTimestamp() }
      }, { merge: true });
    } catch (e) {
  ...
  const renderPage = () => {
    switch (page) {
      ...
      case 'calibration':
        ...
        />;
      case 'history':
        return <HistoryPage db={db} userId={userId} appId={appId} />;
      default:
        return <HomePage
          deviceStatus={deviceStatus}
          liveSweepData={liveSweepData}
  ...
  return (
    <div className="flex min-h-screen w-full bg-gray-900 text-gray-100 font-sans">
      {/* Sidebar */}
      ...
      {/* Main Content Area (scroll removed) */}
...
function HomePage({
  ...
  const isDeviceOnline = (Date.now() / 1000 - (deviceStatus.heartbeat?.seconds || 0)) < 15;
  ...
  const saveSweepToHistory = async () => {
    const dataToSave = dataToDisplay;
          ...
          try {
            const parsedData = results.data.map(row => {
              ...
              if (!freqKey || !magKey || !phaseKey) {
                ...
                if (xKey) throw new Error("This looks like a Tomography file. Please use the Tomography page.");
              ...
              if (isNaN(freq) || isNaN(mag) || isNaN(phase)) throw new Error("Invalid data types in CSV.");
              const phaseRad = phase * (Math.PI / 180.0);
              return { freq, mag, phase, re: mag * Math.cos(phaseRad), im: mag * Math.sin(phaseRad) };
            });
  ...
  const handleClearUpload = () => setUploadedSweepData([]);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gray-800 p-4 rounded-lg shadow-md">
        ...
        <div className="flex flex-wrap gap-4">
          <Button
            onClick={() => { sendDeviceCommand('RUN_SWEEP'); setAnalysisHistory([]); }}
          ...
          />
          <Button
            onClick={() => { sendDeviceCommand('RUN_ANALYSIS'); setAnalysisHistory([]); }}
      ...
      <div className="bg-gray-800 p-4 rounded-lg shadow-md">
        ...
        <div className="flex flex-wrap gap-4">
          <input type="file" accept=".csv, text/csv" onChange={handleFileUpload} className="hidden" ref={fileInputRef} id="csv-upload" />
          <Button onClick={() => fileInputRef.current && fileInputRef.current.click()} text="Upload 1D Data (CSV)" icon={<Upload />} className="bg-teal-600 hover:bg-teal-700" disabled={!isPapaParseLoaded} />
          {uploadedSweepData.length > 0 && (
            <Button onClick={handleClearUpload} text="Clear Uploaded Data" icon={<XCircle />} className="bg-red-600 hover:bg-red-700" />
          )}
        </div>
...
function TomographyPage({ tomoData, setTomoData, isPapaParseLoaded, db, userId, appId }) {
  ...
  const [analysisHistory, setAnalysisHistory] = useState([]);
  const [subject, setSubject] = useState("");
  const [plotMetric, setPlotMetric] = useState('mag');
  const [plotFreq, setPlotFreq] = useState(100);
        ...
        complete: (results) => {
          try {
            ...
            const parsedData = results.data.map(row => {
              ...
              const magKey = Object.keys(row).find(k => k.toLowerCase() === 'impedance' || k.toLowerCase() === 'mag' || k.toLowerCase() === 'magnitude');
              const phaseKey = Object.keys(row).find(k => k.toLowerCase() === 'phase');
              if (!xKey || !yKey || !freqKey || !magKey || !phaseKey) throw new Error("Missing required columns (x, y, frequency, impedance, phase)");
              const freq = parseFloat(row[freqKey]);
              freqs.add(freq);
              return { x: parseFloat(row[xKey]), y: parseFloat(row[yKey]), freq, mag: parseFloat(row[magKey]), phase: parseFloat(row[phaseKey]) };
            });
            const sortedFreqs = Array.from(freqs).sort((a, b) => a - b);
            setAvailableFreqs(sortedFreqs);
            setPlotFreq(sortedFreqs[0] || 100);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gray-800 p-4 rounded-lg shadow-md">
        ...
        <p className="text-sm text-gray-400 mb-3">Upload a CSV file with columns: <code>x</code>, <code>y</code>, <code>frequency</code>, <code>impedance</code>, <code>phase</code></p>
        <div className="flex flex-wrap gap-4">
          <input type="file" accept=".csv, text/csv" onChange={handleTomoFileUpload} className="hidden" ref={fileInputRef} id="csv-tomo-upload" />
          <Button onClick={() => fileInputRef.current && fileInputRef.current.click()} text="Upload 2D Data (CSV)" icon={<Upload />} className="bg-teal-600 hover:bg-teal-700" disabled={!isPapaParseLoaded} />
          {tomoData.length > 0 && <Button onClick={handleClearUpload} text="Clear Uploaded Data" icon={<XCircle />} className="bg-red-600 hover:bg-red-700" />}
          <Button onClick={saveTomoToHistory} text="Save Tomography" icon={<Save />} className="bg-purple-600 hover:bg-purple-700" disabled={tomoData.length === 0 || !subject} />
        </div>
        {!isPapaParseLoaded && <p className="text-xs text-yellow-400 mt-2">Loading data parser...</p>}
      </div>
...
function GeminiAnalysis({
  ...
  const [analysisType, setAnalysisType] = useState(tomoData.length > 0 ? 'soil' : 'general');
...
`,
  ...
  const getHistoricalContext = async () => {
    if (!subject || !db || !userId) return "";
    try {
      const historyCollectionPath = `artifacts/${appId}/users/${userId}/${HISTORY_COLLECTION}`;
      const q = query(collection(db, historyCollectionPath), where("subject", "==", subject), orderBy("createdAt", "desc"), limit(5));
  ...
  const handleApiCall = async (currentChatHistory, systemPrompt, schema) => {
    ...
    for (let i = 0; i < 3; i++) {
      try {
        response = await fetch(GEMINI_API_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
        ...
        if (i === 2) { setError("Network error. Could not connect to the analysis service."); setIsAnalyzing(false); return null; }
        await new Promise(res => setTimeout(res, delay)); delay *= 2;
      }
    }

    if (!response || !response.ok) { setError("Failed to get analysis after retries."); setIsAnalyzing(false); return null; }

    try {
      const result = await response.json();
      const text = result.candidates?.[0]?.content?.parts?.[0]?.text;
      if (!text) { setError("Could not parse the analysis from the response."); return null; }
      if (payload.generationConfig.responseMimeType === "application/json") return JSON.parse(text);
      return text;
    } catch (e) {
  ...
  const handleAnalyze = async () => {
    if (!hasData) { setError("No data available to analyze."); return; }
    setAnalysisHistory([]);
    ...
    if (hasTomoData) {
      ...
      dataString += `Data summary (first 5 points):\n`
        + tomoData.slice(0, 5).map(d => `(x:${d.x}, y:${d.y}) @ ${d.freq}Hz: ${d.mag.toFixed(0)}Ω, ${d.phase.toFixed(1)}°`).join('\n');
    } else {
      dataString = "Type: 1D Frequency Sweep\n"
        + sweepData.map(d => `Freq: ${d.freq.toFixed(0)} Hz, Mag: ${d.mag.toFixed(2)} Ω, Phase: ${d.phase.toFixed(2)}°`).join('\n');
    }
...
Please provide the analysis for the NEW data, comparing it to the historical context if provided.`;
  ...
  const handleFollowUp = async (e) => {
    ...
    setAnalysisHistory(currentChatHistory);
    setUserFollowUp("");
  ...
  return (
    <div className="bg-gray-800 p-4 rounded-lg shadow-md">
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        <AnalysisTypeButton text="General" icon={<FlaskConical />} active={analysisType === 'general'} onClick={() => setAnalysisType('general')} disabled={hasTomoData} />
        <AnalysisTypeButton text="Soil (Moisture/Salinity)" icon={<Droplets />} active={analysisType === 'soil'} onClick={() => setAnalysisType('soil')} />
        <AnalysisTypeButton text="Plant (Stress/Disease)" icon={<Leaf />} active={analysisType === 'plant'} onClick={() => setAnalysisType('plant')} disabled={hasTomoData} />
      </div>

      <Button onClick={handleAnalyze} text="Start New Analysis" icon={<Sparkles />} className="bg-indigo-600 hover:bg-indigo-700" disabled={isAnalyzing || !hasData} />

      {/* Chat history (no inner scrollbar; grows with page) */}
      <div className="mt-4 border-t border-gray-700 pt-4 space-y-4">
        {analysisHistory.map((msg, index) => (<ChatBubble key={index} role={msg.role} content={msg.content} />))}
        {isAnalyzing && (
      ...
      {analysisHistory.length > 0 && (
        <form onSubmit={handleFollowUp} className="mt-4 flex gap-2">
          ...
          />
          <Button text="Send" icon={<Send />} type="submit" className="bg-cyan-600 hover:bg-cyan-700" disabled={isAnalyzing || !userFollowUp.trim()} />
        </form>
      )}
    </div>
  );
...
function HistoryPage({ db, userId, appId }) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      setHistory(historyData); setLoading(false);
    }, (error) => { console.error("Error fetching history:", error); setLoading(false); });
    return () => unsubscribe();
  }, [db, userId, appId]);

  useEffect(() => { chatEndRef.current?.scrollIntoView(); }, [selectedSweep]);
    ...
    <div className="space-y-6">
      ...
      {history.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {/* Details Panel (no inner scroll) */}
          <div className="bg-gray-800 p-4 rounded-lg shadow-md">
            {selectedSweep ? (
              <div className="space-y-6">
                {selectedSweep.type === 'tomo' ? (
                  <div className="space-y-4">
                    <h3 className="text-xl font-semibold text-white">Tomography: {selectedSweep.subject}</h3>
                    <p className="text-sm text-gray-400">
                      Note: This is a simplified static view. Controls for frequency and metric are on the Tomography page.
                    </p>
                    <TomographyHeatmap
                      data={selectedSweep.tomoData.filter(d => d.freq === selectedSweep.tomoData[0].freq)}
                      metric="mag"
                    />
                    <TomographyHeatmap
                      data={selectedSweep.tomoData.filter(d => d.freq === selectedSweep.tomoData[0].freq)}
